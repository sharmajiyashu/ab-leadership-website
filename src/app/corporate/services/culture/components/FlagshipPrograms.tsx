...
export default function FlagshipPrograms({ programs }: FlagshipProgramsProps) {
  ...
  const gridPrograms = programs.slice(0, 5);
        ...
        <div className="space-y-6">
          {fullWidthCard ? (
            ...
            <div className={fullWidthCard === null && !isAnimating ? 'animate-grid-fade-in' : ''}>
              {/*
                3×3 grid (md+): row1 = cards 1–3, row2 = card4 | header | card5.
                Single-column order matches: cards 1–4, header, card 5.
              */}
              <div className="grid grid-cols-1 gap-6 md:grid-cols-3 md:gap-6 items-stretch">
                ...
                >
                  ...
                  <SectionHeading
                    ...
                    subtitle="Comprehensive culture and organizational development programs designed to transform workplace environments"
                    subtitleClassName="max-w-md mb-0"
                  >
                    Our <span className="text-[#0047AB]">Organizational Development</span> Programs
                  </SectionHeading>
                </div>
                {gridPrograms[4] && (
