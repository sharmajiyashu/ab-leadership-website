...
function getFlipCardSubtext(program: FlagshipProgram): string {
  const trimmed = program.subtitle?.trim();
  if (trimmed) return trimmed;
  const first = program.description.split(/\n\n/).filter(Boolean)[0];
  return first ?? program.description;
}
...
export default function FlagshipPrograms({ programs }: FlagshipProgramsProps) {
        ...
        <div className="prose prose-lg max-w-none space-y-6 font-bricolage-text">
          ...
          {program.benefits && program.benefits.length > 0 && (
            <div className="mt-10 pt-8">
              <h3 className="text-2xl font-bold text-gray-800 mb-6 font-bricolage-display">
                Benefits for Employees & the Organisation
              </h3>
  ...
  const renderGridCard = (program: FlagshipProgram) => {
    const subtext = getFlipCardSubtext(program);
    return (
        ...
        >
          ...
          <div className="absolute inset-0 overflow-hidden rounded-3xl backface-hidden [transform:rotateY(180deg)]">
            ...
            <div className="absolute inset-3 sm:inset-4 md:inset-5 z-[2] grid min-h-0 grid-rows-[1fr_auto] rounded-2xl bg-cover bg-center bg-fixed p-3 sm:p-4 bg-[url('/background.svg')]">
              <FlipCardBodyText text={subtext} />
              <div className="flex w-full shrink-0 justify-end pt-2 sm:pt-3">
                <button
                  type="button"
                  className="group animate-what-we-do-cta inline-flex items-center gap-2 bg-white text-gray-800 border-2 border-gray-700 hover:bg-gray-800 hover:text-white hover:border-gray-800 px-7 py-3.5 rounded-full text-base font-medium transition-colors font-bricolage-text cursor-pointer"
  ...
  const gridPrograms = programs.slice(0, 8);
          ...
          {fullWidthCard ? (
            <div className={`bg-white rounded-3xl shadow-xl overflow-hidden ${
              ...
              <div className="relative w-full h-96 md:h-[32rem] overflow-hidden">
                <Image
                  src={programs.find(p => p.id === fullWidthCard)?.image || ''}
            ...
            <div className={fullWidthCard === null && !isAnimating ? 'animate-grid-fade-in' : ''}>
              {/*
                ...
                Single-column order matches: cards 1–4, header, cards 5–8.
              */}
              <div className="grid grid-cols-1 gap-6 md:grid-cols-3 md:gap-6 items-stretch">
                ...
                >
                  ...
                  <SectionHeading
                    ...
                    subtitle="Comprehensive leadership development programs designed to transform leaders at every level"
                    subtitleClassName="max-w-md mb-0"
                  >
                    Our <span className="text-[#0047AB]">Leadership</span> Programs
                  </SectionHeading>
                </div>
                {gridPrograms[4] && (
