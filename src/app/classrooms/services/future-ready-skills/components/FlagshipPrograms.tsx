...
export default function FlagshipPrograms({ programs }: FlagshipProgramsProps) {
  ...
  const gridPrograms = programs.slice(0, 7);
        ...
        <div className="space-y-6">
          {fullWidthCard ? (
            ...
            <div className={fullWidthCard === null && !isAnimating ? 'animate-grid-fade-in' : ''}>
              ...
              */}
              <div className="grid grid-cols-1 gap-6 md:grid-cols-3 md:gap-6 md:items-stretch">
                {gridPrograms[0] && (
                ...
                >
                  ...
                  <SectionHeading
                    ...
                    subtitle="Comprehensive future-ready skills programs designed to prepare students for tomorrow&apos;s challenges"
                    subtitleClassName="max-w-md mb-0"
                  >
                    Our <span className="text-[#0047AB]">Future Ready Skills</span> Programs
                  </SectionHeading>
                </div>
                {gridPrograms[4] && (
                ...
                {(gridPrograms[5] || gridPrograms[6]) && (
                  <div className="md:col-span-3 flex flex-col md:flex-row justify-center gap-6">
                    {gridPrograms[5] && (
                      <div key={gridPrograms[5].id} className="min-h-0 w-full md:w-[calc((100%-3rem)/3)]">
                        {renderGridCard(gridPrograms[5])}
                      </div>
                    )}
                    {gridPrograms[6] && (
                      <div key={gridPrograms[6].id} className="min-h-0 w-full md:w-[calc((100%-3rem)/3)]">
