...
import { TeacherTrainingProgram } from '../types';
import SectionHeading from '@/components/sections/SectionHeading';

interface TeacherTrainingProgramsProps {
  programs: TeacherTrainingProgram[];
}

function getFlipCardBodyText(program: TeacherTrainingProgram): string {
  return program.description.split(/\n\n/).filter(Boolean).join(' ');
}
...
export default function TeacherTrainingPrograms({ programs }: TeacherTrainingProgramsProps) {
  ...
  const renderGridCard = (program: TeacherTrainingProgram) => {
        ...
        <div className="space-y-6">
          {fullWidthCard ? (
              ...
              <div className="p-8 md:p-12">
                <div className="max-w-4xl mx-auto">
                  <button
                    ...
                    className="mb-6 text-gray-600 hover:text-gray-800 transition-colors font-medium font-bricolage-text"
                  >
                    ← Back to Programs
                  </button>
            ...
            <div className={fullWidthCard === null && !isAnimating ? 'animate-grid-fade-in' : ''}>
              {/*
                3×3 grid (md+): row1 = cards 1–3, row2 = card4 | header | card5, row3 = cards 6–7 centered.
                Single-column order matches: cards 1–4, header, cards 5–7.
              */}
              <div className="grid grid-cols-1 gap-6 md:grid-cols-3 md:gap-6 items-stretch">
                ...
                <div
                  ref={textRef}
                  className="relative isolate flex h-full min-h-[280px] md:min-h-[300px] flex-col justify-center items-center text-center px-4 py-8 md:py-0 transition-all duration-700 ease-out"
                  style={{
                    opacity: textOpacity,
                ...
                >
                  ...
                  <SectionHeading
                    ...
                    subtitle="Comprehensive professional development programs designed to enhance teaching effectiveness and student success"
                    subtitleClassName="max-w-md mb-0"
                  >
                    Our <span className="text-[#0047AB]">Teacher and Parent Training</span> Programs
                  </SectionHeading>
                </div>
                {gridPrograms[4] && (
                ...
                {(gridPrograms[5] || gridPrograms[6]) && (
                  <div className="md:col-span-3 grid grid-cols-1 md:grid-cols-6 gap-6">
                    {gridPrograms[5] && (
                      <div key={gridPrograms[5].id} className="min-h-0 md:col-start-2 md:col-span-2">
                        {renderGridCard(gridPrograms[5])}
                      </div>
                    )}
                    {gridPrograms[6] && (
                      <div key={gridPrograms[6].id} className="min-h-0 md:col-span-2">
