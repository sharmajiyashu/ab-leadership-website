...
export default function FlagshipPrograms({ programs }: FlagshipProgramsProps) {
  ...
  const gridPrograms = programs.filter(p => p.id !== 7 && p.id !== 8);
        ...
        <div className="space-y-6">
          {fullWidthCard ? (
            ...
            <div className={fullWidthCard === null && !isAnimating ? 'animate-grid-fade-in' : ''}>
              {/*
                3×3 grid (md+): row1 = cards 1–3, row2 = card4 | header | card5, row3 = card6 centered.
                Single-column order matches: cards 1–4, header, cards 5–6.
              */}
              <div className="grid grid-cols-1 gap-6 md:grid-cols-3 md:gap-6 items-stretch">
                ...
                >
                  ...
                  <SectionHeading
                    ...
                    subtitle="Comprehensive mental health and wellbeing programs designed to support students&apos; emotional, psychological, and social development"
                    subtitleClassName="max-w-md mb-0"
                  >
                    Our <span className="text-[#0047AB]">Mental</span> Health Programs
                  </SectionHeading>
                </div>
                {gridPrograms[4] && (
                ...
                {gridPrograms[5] && (
                  <div key={gridPrograms[5].id} className="min-h-0 md:col-start-2">
