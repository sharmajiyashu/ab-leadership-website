      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        >
          <SectionHeading
            ...
            subtitle="Trauma-informed creative and body-centered therapeutic programs that support healing, empowerment, and restoration"
          >
            Our <span className="text-[#0047AB]">Women in Shelters</span> Programs
          </SectionHeading>
        </div>
        ...
        <div>
            ...
            {programs.slice(0, 3).map((program, index) => (
              ...
              >
                <div className="relative w-full h-48 overflow-hidden">
                  <Image
                    src={`/corporate/gallery/${(index % 10) + 1}.jpg`}
                    alt={program.title}
                    fill
          ...
          {/* Second row - 4 programs */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mt-6">
            {programs.slice(3, 7).map((program, index) => (
              <div
                key={program.id}
                className="bg-white rounded-lg shadow-md transition-all duration-300 overflow-hidden cursor-default hover:shadow-lg hover:transform hover:scale-102"
              >
                <div className="relative w-full h-48 overflow-hidden">
                  <Image
                    src={`/corporate/gallery/${(index + 3) % 10 + 1}.jpg`}
                    alt={program.title}
                    fill
                    className="object-cover"
                  />
